...
function describeWebAuthnError(
  ...
  if (
    ...
    return {
      ...
      message:
        "Passkey 被浏览器安全策略拒绝。请确认正在正式 HTTPS 网站操作，然后重新加载页面再试。"
  ...
  if (
    ...
    return {
      ...
      message:
        "这台设备当前没有找到可用的 Passkey，或者你取消了系统提示。新设备可以先使用设备配对；登录后再到 Passkey 安全中心测试或添加本机凭证。"
  ...
  async function () {
    ...
    try {
      ...
      if (
        ...
        setStatus(
          "Passkey 已由设备返回，但服务器验证没有通过。请重新尝试；如果只在某一台设备失败，请先用设备配对登录，再到 Passkey 安全中心测试该设备。",
          "error"
        );
...
<div class="wrap">
  ...
  <section class="card section">
    ...
    <p class="muted">
      ...
      真正增加独立备份时，应选择不同的 authenticator、密码管理器、另一台设备或安全密钥。
  ...
  <section class="card section">
    ...
    class="notice">
      ...
      私钥、指纹、Face ID、Windows Hello PIN 都不会上传到服务器。

      Jingyan Media Center 只保存验证所需的 credential ID、公钥和认证状态。
  ...
  if (
    ...
    if (
      ...
      securityNotice.textContent =
        "Owner 当前至少有一把已备份/同步型 Passkey。它可能已经能在多台兼容设备上使用，因此数据库里只有一条 credential 不等于只有一台设备可登录。仍建议保留另一条独立恢复路线。";
